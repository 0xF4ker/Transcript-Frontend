...
const Colleges = () => {
	...
	const [createCollege, { isLoading, isError, error, isSuccess }] =
		...
		const [editCollege, { isLoading: isLoadingEdit, isError: isErrorEdit, error: errorEdit, isSuccessEdit }] =
		useEditCollegeMutation();

		const [deleteCollege, { isLoading: isLoadingDelete, isError: isErrorDelete, error: errorDelete, isSuccess: isSuccessDelete }] =
		useDeleteCollegeMutation();

		useEffect(() => {
			if (isSuccessDelete) {
				toast.success("College delete succesfully");
				setHide(false);
			}
			if (isErrorDelete) {
				...
				if ((error as any)?.data) {
					...
					toast.error("College delete failed", {
						position: "top-right",
					});
				}
			}
		...
		useEffect(() => {
			if (isSuccessEdit) {
				toast.success("College edited succesfully");
				setHide(false);
			}
			if (isErrorEdit) {
				...
				if ((error as any)?.data) {
					...
					toast.error("College edit failed", {
						position: "top-right",
					});
				}
			}
	...
	const { data, isLoading: isLoadingColleges } = useGetCollegesQuery("");
										...
										<tr key={id}>
											...
											<td className="text-center">
												<ul className="table-controls">
														...
														}} data-bs-toggle="modal" data-bs-target="#editCollege"	className="bs-tooltip"  data-bs-placement="top" title="Edit" data-original-title="Edit"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className="feather feather-edit-2 p-1 br-8 mb-1"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path></svg></a></li>
                                                        <li><a onClick={()=> {
													...
													<div className="modal fade" id="editCollege" tabIndex={-1} role="dialog" aria-labelledby="editCollegeTitle" aria-hidden="true">
                                        <div className="modal-dialog modal-dialog-centered modal-xl" role="document">
                                            <div className="modal-content">
                                                <div className="modal-header">
                                                    <h5 className="modal-title" id="editCollegeTitle">Edit College</h5>
								...
								>
									<div className="col-12">
										...
										<div className="input-group">
											...
											/>
											<input hidden {...registerEdit("id", {required: true})}/>
										</div>
									</div>
									...
									<div className="modal fade" id="deleteCollege" tabIndex={-1} role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                                        <div className="modal-dialog modal-dialog-centered" role="document">
                                            <div className="modal-content">
                                                <div className="modal-header">
                                                    <h5 className="modal-title" id="exampleModalCenterTitle">Delete College</h5>
                                                    <button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Close">
                                                      <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className="feather feather-x"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
                                                    </button>
                                                </div>
